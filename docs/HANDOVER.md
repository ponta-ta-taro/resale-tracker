...
- [x] **在庫管理UI構築**
  ...
  - [x] ステータス進捗バーコンポーネント
  - [x] ヘッダーナビゲーション統一
- [x] **Phase 2: メール自動取り込み（進行中）**
  - [x] /api/mail/webhook 全面改修
  - [x] Fromヘッダーでユーザー特定（Gmail転送対応）
  - [x] 注文確認メールのパース・在庫登録
  - [x] 複数商品対応（item_index）
  - [x] types/index.ts の型定義更新（serial_number, imei等追加）
  - [x] 実メールでのテスト成功（W1699438321: 2台登録）
  - [x] email_logs テーブルへのログ記録（実際のDB構造に合わせて修正）
  - [x] 在庫詳細ページのReactエラー修正（データサニタイゼーション追加）
  - [ ] 出荷通知メールのテスト（Gmail届き次第）

### 🔄 次にやること

1. **出荷通知メールのテスト**（Gmailに届いたら）
2. **email_logs の確認・UIの作成**
3. **請求書PDF対応**（シリアル番号登録）
...
| 2026/01/18 | Phase 2 進行中：注文確認メール自動取り込み完成、テスト成功 |
| 2026/01/18 | email_logs記録機能を実際のDB構造に合わせて修正 |
| 2026/01/18 | 在庫詳細ページのReactエラー修正（データサニタイゼーション） |
