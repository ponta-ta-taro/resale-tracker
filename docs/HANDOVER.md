...
- [x] **iCloud転送テスト成功**: colorフィールドが正しく抽出されることを確認
- [x] **Amazonメール構造分析**: 4種類のメール（注文・発送・配達中・配達完了）の構造を解析
- [x] **Amazonパーサー実装完了**: 注文確認・発送・配達メール対応
- [x] **Amazonパーサーテスト完了**: 全フィールド正常抽出確認（01_order_250-8477857-2415055.eml）
- [x] **価格抽出バグ修正**: カンマなし形式対応、最大値選択ロジック追加（¥800 → ¥179,800）
- [x] **Amazonパーサー本番テスト完了**:
  - `source` → `purchase_source` カラム名修正
  - `arrived_at` → `delivered_at` カラム名修正
  - ステータス値修正（`arrived` → `delivered`）
  - 注文確認・発送・配達完了メール全て成功
- [x] **Vercelビルドエラー修正**: tsconfig.jsonのexcludeにscriptsフォルダ追加
...
1. **買取販売情報の記入・テスト**
   - 在庫詳細ページで買取情報を手入力
   - ステータス遷移の確認（delivered → sent_to_buyer → buyer_completed → paid）

2. **仕入価格・注文日の一覧表示問題**
   - 詳細画面では値があるが、一覧では「-」表示になる
...
| Amazon → Gmail | ✅ | ✅ | 注文確認・発送・配達完了すべてOK |
...
### Amazon価格抽出の注意点
- Amazonメールは `179800 JPY`（カンマなし）形式
- Appleメールは `¥179,800`（カンマあり）形式
- 正規表現で両方対応: `/(\d{1,3}(?:,\d{3})+|\d{5,})\s*(?:円|JPY)/g`
- 複数マッチ時は最大値を選択（商品価格 vs 送料等の区別）
- 修正前は ¥800 を誤抽出していた → 修正後 ¥179,800 を正しく抽出
...
### 買取発送の配送追跡
買取業者への発送時、送り状番号を登録して配送状況を自動追跡する機能

**要件:**
- 送り状番号（tracking_number）を在庫に紐付けて登録
- 配送業者を選択（郵便局 / ヤマト / 佐川急便）
- 各社の追跡ページへのリンク or 状況自動取得
