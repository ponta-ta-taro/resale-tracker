...
**目的**
Appleのゲスト注文ステータスページに直接アクセスするためのトークンを保存。
将来的にスクレイピングで注文ステータスを自動取得する予定。

**仕組み**
- 注文確認メールのリンクにトークンが含まれる
- 例: `/shop/order/guest/W1528936835/{トークン}?e=true`
- メール処理時に正規表現で抽出し `inventory.order_token` に保存

**注意**
- お届け日変更メール等のトークンは認証が必要で直接アクセス不可
- 注文確認メールのトークンのみが直接アクセス可能
