...
export default function ShippingAddressesManager() {
    ...
    const resetForm = () => {
        setFormData({ name: '', postal_code: '', address: '', address_variant: '', phone_number: '', notes: '' });
        setEditingId(null);
        setShowForm(false);
    };

    const handleEdit = (item: ShippingAddress) => {
        setFormData({
            name: item.name,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                ...
                address_variant: formData.address_variant || null,
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const response = await fetch(`/api/shipping-addresses/${id}`, { method: 'DELETE' });
            if (response.ok) {
                await fetchItems();
            } else {
                alert('削除に失敗しました');
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-gray-900">配送先住所一覧</h2>
                <button
            ...
            {showForm && (
                <div className="bg-white p-6 rounded-lg shadow mb-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">表記バリエーション</label>
                                <input
                                    type="text"
                                    value={formData.address_variant}
                                    onChange={(e) => setFormData({ ...formData, address_variant: e.target.value })}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-md"
                                    placeholder="6丁目 / 6- など"
                                />
                            </div>
                            <div>
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">宛名</th>
                            ...
                            items.map((item) => (
                                <tr key={item.id} className={`hover:bg-gray-50 ${!item.is_active ? 'opacity-50' : ''}`}>
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600">
                                        <div>{item.postal_code && `〒${item.postal_code}`}</div>
